...
abstract class Sub_Data_Categorie_Lot_Categorie extends Compareable_Is_Operand implements I_Filename_Shema {
  ...
  private const array_option_id = [
    "option_residential" => "RE",
    "option_community" => "CO",
  ];

  # input shema template for ui
  private const input_shema_template = '
  <div class="container_label_and_input sub_input option_lot_sub_data_categorie%1$d">
    <label for="%3$s%1$d">Lot-Kategorie</label>
    <select class="%3$s%1$d option_lot_sub_data_categorie%1$d" name="%2$s[%1$d]['.self::class.']" id="%3$s%1$d" required>
      <option value="" selected disabled>Auswählen</option>
      <option value="option_residential">Wohngrundstück</option>
      <option value="option_community">Gemeinschaftsgrundstück</option>
    </select>
    <script>
      document.addEventListener("DOMContentLoaded", function(){
        setTimeout(function(){
          disable_and_hide_input_by_class_name_if_source_element_is_not_selected(\''.Filename_Shema_Categorie::class.'%1$d\', \'option_lot\', \'option_lot_sub_data_categorie%1$d\');
        },100);
      });
    </script>
  </div>
  ...
  <div class="container_label_and_input sub_input option_lot_sub_data_categorie%1$d">
    ...
    <select class="%3$s%1$d option_lot_sub_data_categorie%1$d" name="%2$s[%1$d]['.Ui::ui_search_data_key_value_root.']['.self::class.'][]" id="%3$s%1$d" required>
      <option value="" selected disabled>Auswählen</option>
      <option value="option_residential">Wohngrundstück</option>
      <option value="option_community">Gemeinschaftsgrundstück</option>
    </select>
