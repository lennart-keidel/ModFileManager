...
abstract class Filename_Shema_Installation_Date extends Compareable_Date_Operand implements I_Filename_Shema {
  ...
  private const input_shema_template = '
    <div class="container_label_and_input">
      ...
      <input class="%4$s%1$d" id="'.self::class.'%1$d" type="date" name="%2$s[%1$d]['.self::class.']" value="%3$s" %5$s>
    </div>
  ';
  ...
  public static function print_filename_shema_input_for_ui(int $index, string $different_ui_key_root = null, bool $is_required = true) : void {
    printf(self::input_shema_template, $index, ($different_ui_key_root === null ? Ui::ui_data_key_root : $different_ui_key_root), (new DateTime())->format('Y-m-d'), self::class, ($is_required === true ? "required" : ""));
  }
